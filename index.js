...
function addArtist(array, aname, theyears, agenre, thenationality, abio) {
  
  array.push({'name': aname, 'years': theyears, 'genre': agenre, 'nationality': thenationality, 'bio': abio})
...
function lotsOfArt(array) {
  ...
  const filteredNamesArray = [];

  for (let i = 0; i < array.length; i++) {
    if (array[i].paintings > 100){
      filteredNamesArray.push(array[i].name)
    }
  }
